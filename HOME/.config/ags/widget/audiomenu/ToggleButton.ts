...
App.connect('window-toggled', (_, name: string, visible: boolean) => {
    if (name === 'audio' && !visible) Utils.timeout(500, () => (opened.value = ''));
});
...
export const SimpleMenu = ({ name, icon, title, content }: SimpleMenuProps) =>
    Widget.Box({
        child: Widget.Box({
            class_names: ['menu', name],
            vertical: true,
...
type AdvancedToggleButtonProps = {
    name: string;
    icon: IconProps['icon'];
    label: LabelProps['label'];
    activate: () => void;
...
export const AdvancedToggleButton = ({
    ...
    Widget.Box({
        class_name: class_name || 'toggle-button',
        setup: (self) =>
            self.hook(service, () => {
                self.toggleClassName('active', condition());
            }),
